...
const handleNavbarToggle = () => {
  ...
  return (
    <header className={'top-0 left-0 p-4 flex w-full items-center  bg-transparent  ${sticky ? "!fixed !z-[9999] !bg-opacity-80 shadow-stickv backdrop-bluf-sm !transistion dark:!bg-dark dark:!bg-opacity-20" : "absolute"}'}>
        <div className='container'>
            <div className='relative -mx-4 flex items-center justify-between'>
                <div className='w-60 max-w-full px-4 xl:mr-12'>
                    <Link href={"/"} className={'text-[30px] font-extrabold cursor-pointer block w-full ${sticky ? "py-5 lg:py-2" : "py-8"}'}>STSBlog</Link>
                </div>

                <div className='flex w-full items-center justify-between px-4'>
                    <div>
                        <button onClick={handleNavbarToggle} id='navbarToggle' aria-label='Mobile Menu' className='absolute right-4 top-1/2 block translate-y-[-50%] rounded-lg px-3 py-[6px] ring-primary focus:ring-2 lg:hidden'>
                        ...
                        <nav id='navbarCollapse' className={'flex items-center absolute right-0 z-30 w-[250px] rounded border-[.5px] bg-white border-body-color/50 py-4 px-6 duration-300 dark:border-body-color/20 dark:bg-dark lg:visible lg:static lg:w-auto lg:border-none lg:!bg-transparent lg:p-0 lg:opacity-100  ${navbarOpen ? "visible top-full opacity-100" : "invisible top-[120%] opacity-0" }'}>
                            <ul className='block lg:flex lg:space-x-12'>
                                {menuItems.map((item : MenuItems)=><li key={item.id} className='group relative'>
                                        <Link href={item.path} className={'flex py-2 text-dark group-hover:opacity-70 dark:text-white lg:inline-flex lg:py-6 lg:px-0'}>
                                        {item.label} </Link>
                                    </li>)}
                            </ul>
                        </nav>
